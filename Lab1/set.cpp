...
Set Set::operator*(const Set& b) const
{
    ...
    Node* pt = T.head; //pt pekar på dummy node
    Node* p = head->next; //pekar på noden efter dummy node

    while (p != nullptr){
        if(b.member(p->value)) {
            T.insert(pt, p->value);
            pt = pt->next; //pt pekar nu på det tillagd objektet
        }
        p = p->next; //p går fram ett steg
    }
	return T;
}
